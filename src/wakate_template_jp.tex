...
% 代数多様体とは、大雑把にいうと「多項式からなる連立方程式で定義される図形」のことである。
% 例えば直線$y = x$や放物線$y = x^2$は代数多様体である。
% この節では楕円曲線およびその「退化」である小平ファイバーを例として取り上げながら、代数幾何学の基本的な概念を紹介する。

% 楕円曲線とは三次式で定義される曲線のうち、特異点を持たないもののことである。
% 例えば曲線$y^2 = x^3 + x$は楕円曲線である。$y^2 = x^3 + x^2$は三次式で定まるが、$f = y^2-x^3 - x^2$のJacobi行列$(-3x^2-2x, 2y)$は原点に特異点を持つので楕円曲線ではない（下図を参照）。

% TODO: 楕円曲線の例を図にする


% より正確に述べよう。
% \textbf{以下この節では方程式の係数も点の座標もすべて複素数}とする。
% 代数多様体$X$が1つの方程式$f(x_1, \dots, x_n) = 0$で定まるとき、$X$の\textbf{特異点}とは$f$のJacobi行列$Jf$のランクが$0$になる点$x \in X \subset \bC^n$のことである（連立方程式の場合も同様に定義される）。
% すると陰関数定理より、$X$の非特異点$x$の近傍では$X$は複素多様体となる。その次元が$1$のとき曲線、$2$のとき曲面という。
% 特異点を持たない代数多様体を\textbf{非特異}という。

% 複数の代数多様体を多項式関数や有理関数による座標変換で貼り合わせたものもまた代数多様体である。
% \textbf{射影直線}$\bP^1 = (\bC^2 \setminus \{0\}) / \bC^\times = \{[x_0:x_1]\}$は$\{[z:1] \mid z \in \bC\}$と$\{[1:w] \mid w \in \bC\}$という2つの$\bC$の貼り合わせで得られる。
% 座標変換$w = 1/z$は有理関数だから、$\bP^1$は代数多様体である。
% 同様に\textbf{射影平面}$\bP^2$や\textbf{射影空間}$\bP^n$が定義される。
% 射影空間の中で斉次多項式の零点として表される図形を\textbf{射影多様体}という。
% 例えば$y^2 = x^3 + x$は$\bC^2$の方程式だが、$\bC^2 = \{[x:y:1] \mid x, y \in \bC\} \subset \bP^2$とみなすと$(x_1/x_2)^2 = (x_1/x_2)^3 + (x_1/x_2)$となり、分母をはらって$x_1^2x_2 = x_1^3 + x_1x_2^2$とすれば$\bP^2$の方程式に変換できる。
% 楕円曲線の定義は$\bP^2$の三次斉次多項式から得られる非特異射影代数多様体\footnote{正確にはその上の1点も指定する必要がある。}である。

% 非特異射影多様体は最も基本的なクラスの代数多様体で、

% 代数多様体の定義方程式をパラメータを使って変化させることで、\textbf{代数多様体の族}を考えることができる。
% 例として、多項式$y^2 = x^3 + t$を考える。
% これを$(t, x, y)$を座標にもつ3次元空間$\bC^3$の中の曲面だと思うと非特異だが、パラメータ$t$ごとに定まる三次曲線の族とみなすと、$t = 0$のとき特異点をもち$t \neq 0$のとき楕円曲線となる。
% つまり楕円曲線の族$\{E_t\}_t$が$t \to 0$の極限で特異点を持つ曲線に退化している。
% 一般に非特異射影曲面$S$から非特異射影曲線$C$への全射$\pi \colon S \to C$で、有限個の$t \in C$を除いてファイバー$\pi^{-1}(t)$が楕円曲線になるようなものを\textbf{楕円曲面}という。
% その特異ファイバーとして現れる曲線は小平とN\'{e}ronにより分類されていて、\textbf{小平ファイバー}と呼ばれる。
